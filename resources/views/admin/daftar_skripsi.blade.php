            ...
            <div class="row">
                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="/admin/page_pra_sempro">Pra Seminar Proposal</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Judul Skripsi</li>
                        </ol>
                    </nav>
                </div>
            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <table>
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <form action="{{route('editSkripsi')}}" method="POST">
                                                                @csrf
                                                                <input type= "hidden" name= "nim" value= "{{$skripsi->nim}}">
                                                                <input type= "hidden" name= "nama" value= "{{$skripsi->nama}}">
                                                                <button type="submit" class="btn btn-warning btn-sm"><i class="bi bi-pencil-square"></i></button>
                                                            </form>
                                                        </td>
                                                        <td>
                                                            <form action="{{route('deleteSkripsi')}}" method="POST">
                                                                @csrf
                                                                <input type="hidden" name="nim" value="{{$skripsi->nim}}">
                                                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus judul skripsi? Anda tidak dapat mengembalikan data yang telah dihapus.')"><i class="bi bi-trash"></i></button>
                                                            </form>
                                                        </td>
                                                    </tr>
                                                    @else
                                                    <tr>
                                                        <td>
                                                            <form action="{{route('regisSkripsi')}}" method="get">
                                                                @csrf
                                                                <input type= "hidden" name= "nim" value= "{{$skripsi->nim}}">
                                                                <input type="hidden" name="nama" value="{{$skripsi->nama}}">
                                                                <button type="submit" class="btn btn-success btn-sm"><i class="bi bi-plus-square"></i> Daftarkan</button>
                                                            </form>
                                                        </td>
                                                    </tr>
                                                    @endif
