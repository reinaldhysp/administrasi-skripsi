                        ...
                        <div class="card card-outline-secondary">
                            <div class="row align-items-center m-5">
                                <div class="col-md mb-5">    
                                    ...
                                    <form class="form form-horizontal" method="post" action="{{route('cetakUndanganSemhas')}}">
                                        @csrf
                                        <div class="form-body">
                                            <div class="row">
                                                <div class="col-md-4">
                                                ...
                                                <div class="col-md-6">
                                                    <select class="form-control  @error('tanggal_semhas') is-invalid @enderror" id="tanggal_semhas" name="tanggal_semhas" required value="{{ old('tanggal_semhas') }}" autocomplete="tanggal_semhas">
                                                        <option value="">-- Pilih Jadwal Semhas --</option>
                                                        <?php $i ?>
                                                        @for($i=0; $i<=(count($query))-1; $i++)
                                                            <option  value="{{$query[$i]->tanggal_semhas}}">{{Carbon::parse($query[$i]->tanggal_semhas)->translatedFormat('l / d F Y')}}</option>
                                                        @endfor
                                                    </select>
                                    ...
                                    <form class="form form-horizontal" method="post" action="{{route('cetakJadwalSemhas')}}">
                                        @csrf
                                        <div class="form-body">
                                            <div class="row">
                                                <div class="col-md-4">
                                                ...
                                                <div class="col-md-6">
                                                    <select class="form-control  @error('tanggal_semhas') is-invalid @enderror" id="tanggal_semhas" name="tanggal_semhas" required value="{{ old('tanggal_semhas') }}" autocomplete="tanggal_semhas">
                                                        ...
                                                        @for($j=0; $j<=(count($query))-1; $j++)
                                                            <option  value="{{$query[$j]->tanggal_semhas}}">{{Carbon::parse($query[$j]->tanggal_semhas)->translatedFormat('l / d F Y')}}</option>
                                                        @endfor
                                                    </select>
                                                    @error('tanggal_semhas')
                                                        <div class="text-danger">{{ $message }}</div>
