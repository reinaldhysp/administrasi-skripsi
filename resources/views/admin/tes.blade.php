                               ...
                               @if(session('status'))
                                ...
                                <table class="table table-bordered mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{$k}}</td>
                                            <td class="text-bold-500">{{$mahasiswas[$i]->nama_mhs}} ({{$mahasiswas[$i]->nim}})</td>
                                            <td colspan="3">
                                                @if($mahasiswas[$i]->nama != NULL)
                                                    @for($j=$i; $j<4; $j++)
                                                    <p>{{$mahasiswas[$j]->nama}} : {{$mahasiswas[$j]->total}}</p> 
                                                    @endfor
                                                @else
                                                    <center><p> <i>Mahasiswa Belum Memiliki Nilai !</i> </p></center>
                                                @endif
                                            </td>
                                            <td>
                                                <center>
                                                <table>
                                                    ...
                                                    @if($mahasiswas[$i]->total != NULL)
                                                        <td>
                                                            <form action="{{route('edit_nilai_sidang_admin')}}">
                                                                @csrf
                                                                <input type="hidden" name="nim" value="{{$mahasiswas[$i]->nim}}">
                                                                <button type="submit" class="btn btn-warning btn-sm"><i class="bi bi-pencil-square"></i></button>
                                                            </form>
                                                        </td>
                                                        <td>
                                                            <form action="{{route('delete_nilai_sidang_admin')}}" method="POST">
                                                                @csrf
                                                                <input type="hidden" name="nim" value="{{$mahasiswas[$i]->nim}}">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <form action="{{route('add_nilai_sidang_admin')}}" method="GET">
                                                                @csrf
                                                                <input type= "hidden" name= "nim" value= "{{$mahasiswas[$i]->nim}}">
